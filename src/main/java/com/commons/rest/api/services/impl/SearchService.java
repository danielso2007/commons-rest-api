...
public class SearchService<E extends BaseEntity, K extends Serializable, R extends IBaseRepository<E, K>>
	...
	public Sort getSort(Map<String, String> filters) {
		...
		String[] fields = null;
		fields = orderBy[0].split(IConstantsAPI.COMMA);

		List<Order> orders = new ArrayList<>();

		for (int x = 0; x < fields.length; x++) {
			String[] field = fields[x].split(IConstantsAPI.COLON);
			if (field.length == 2) {
	...
	public Criteria getCriteriaByFilter(Criteria criteria, final String filter, final Object value, String regex) {
		...
		} else if (filter.equalsIgnoreCase(IConstantsAPI.GREATER_THAN)) {
			return criteria.gt(value);
		} else if (filter.equalsIgnoreCase(IConstantsAPI.LESS_THAN)) {
			return criteria.lt(value);
		} else if (filter.equalsIgnoreCase(IConstantsAPI.GREATER_THAN_OR_EQUAL)) {
			return criteria.gte(value);
		} else if (filter.equalsIgnoreCase(IConstantsAPI.LESS_THAN_OR_EQUAL)) {
			return criteria.lte(value);
		} else if (filter.equalsIgnoreCase(IConstantsAPI.EQ)) {
			return criteria.is(value);
		} else if (filter.equalsIgnoreCase(IConstantsAPI.CONTAINS)) {
			return criteria.regex(String.format(IConstantsAPI.REGEX_CONTAINS, value));
		} else {
			return criteria.is(value);
		}
	}
	...
	public Criteria createCriteriaByFilter(final Query query, final String field, final String value, String regex) {
		String[] filters = value.split(IConstantsAPI.COLON);
		...
		if (filters.length == 2) {
			...
			bsonList.add(getCriteriaByFilter(criteria, filters[2], getValueByFieldType(field, filters[3]), regex)
					.getCriteriaObject());
			return new Criteria(IConstantsAPI.AND).is(bsonList);
		} else {
